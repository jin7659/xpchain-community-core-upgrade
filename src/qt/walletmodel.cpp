...
{
    if (success) {
        Q_EMIT message(tr("Rescan Complete"),
                       tr("Blockchain rescan finished. Check Overview for your balance."),
                       CClientUIInterface::MSG_INFORMATION);
    } else {
        Q_EMIT message(tr("Rescan Failed"),
                       tr("The wallet could not start or complete a blockchain rescan.\n\n"
                          "If another rescan is already running, wait for it to finish or cancel it from the progress dialog, then try Tools → Rescan Wallet again."),
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered xpchain address / amount:
...
bool WalletModel::bumpFee(uint256 hash)
{
    ...
    questionString.append(XPChainUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(XPChainUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(XPChainUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));

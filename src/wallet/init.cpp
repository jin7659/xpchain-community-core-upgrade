...
{
    gArgs.AddArg("-addresstype", strprintf("What type of addresses to use (\"legacy\", \"p2sh-segwit\", \"bech32\", or \"bech32m\", default: \"%s\"). \"bech32m\" is ignored until Taproot activates", FormatOutputType(DEFAULT_ADDRESS_TYPE)), false, OptionsCategory::WALLET);
    ...
    gArgs.AddArg("-changetype", "What type of change to use (\"legacy\", \"p2sh-segwit\", \"bech32\", or \"bech32m\"). Default is same as -addresstype, except when -addresstype=p2sh-segwit a native segwit output is used when sending to a native segwit address). \"bech32m\" is ignored until Taproot activates", false, OptionsCategory::WALLET);
...
{
    for (const std::shared_ptr<CWallet>& pwallet : GetWallets()) {
        pos::MintStake(threadGroup, pwallet);
    }
}
